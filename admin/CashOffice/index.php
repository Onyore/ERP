...
<html>
  ...
  <script type="text/javascript" src="http://canvg.github.io/canvg/canvg.js"></script>
    ...
    <title>CASH OFFICE DEPARTMENT</title>
...
$sql="SELECT*FROM students where Cash_Status=0 and NOT Library_Status=0";
 //<a href='delete.php? id=".$row['id']."';> Delete</a></td><tr>";
$result=mysqli_query($con,$sql);

while($row=mysqli_fetch_array($result)){
$Transcript=$row['Transcript_Fee'];
$Library_Reason=$row['Library_Reason'];
$Gown_Fine=$row['Gown_Fine'];
$Finance_Reason=$row['Finance_Reason'];
$Total=$Transcript + $Library_Reason + $Gown_Fine + $Finance_Reason;
...
elseif($School==12){
	...
	$School="SCHOOL OF PURE AND APPLIED SCIENCES";
            ...
            echo"<tr><td>".$row['id']."</td><td>".$row['RegNo']."</td><td>".$row['FullName']."</td><td>".$Transcript."</td><td>"
			.$Library_Reason."</td> <td>".$Gown_Fine."</td>
			
			<td>".$Total."</td>
			<td><a onclick='openForm()'><u>Receipt</u></a></td>
			 </tr>";
...
<div class="form-popup" id="myForm">
  <form action="Receipt.php" class="form-container" method=GET>
    <h1>Receipt(Clear)</h1>
    <label for="Fine"><b>Receipt Number</b></label>
    <input type="text" placeholder="Enter receipt number" name="Receipt" required>
